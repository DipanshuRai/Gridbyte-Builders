...
export const getAllReviews = (id) => async (dispatch) => {
    try {
        ...
        const { data } = await axiosInstance.get(`/api/v1/admin/reviews?id=${id}`);
...
export const deleteReview = (reviewId, productId) => async (dispatch) => {
    try {
        ...
        const { data } = await axiosInstance.delete(`/api/v1/admin/reviews?id=${reviewId}&productId=${productId}`);
